...
• Команда (8, 🔔 1 упоминание)
• Работа (42)
• Мониторинг (23)
• Новости (127)

📊 Всего: 5 каналов, 215 непрочитанных, 🔔 4 упоминания
```

**Особенности команды `/unread`:**
- Показывает только каналы и группы (личные переписки не показываются)
- **Не показывает замьюченные каналы** (только активные каналы)
- Каналы с упоминаниями (@username) показываются первыми и помечены 🔔
- В скобках: количество непрочитанных сообщений и количество упоминаний
- Отсортировано: сначала каналы с упоминаниями, затем по количеству непрочитанных
...
```
**Результат:** Только непрочитанные сообщения из этого чата

```
Суммаризируй непрочитанное из чата Работа
```
**Результат:** Анализ всех непрочитанных сообщений

```
Что нового среди непрочитанных в чате Команда?
```
**Результат:** Обзор непрочитанных сообщений

⚠️ **Важно:** Загрузка сообщений через бота **НЕ отмечает их как прочитанные**. Они останутся непрочитанными в вашем основном Telegram.
...
# Использует DeepSeek для парсинга команды
# Использует Alice AI для анализа
# Сохранит контекст
